    ...
    <div className="modal-overlay">
      <div className="modal-content">
        <div className="modal-header">
          <h3>Manage members: {activeSpace.name}</h3>
          ...
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              <input
                ...
                required
              />
              <small className="field-help">The user must be registered. They become a member only after accepting the invitation in Notifications.</small>
            </div>
            <div className="form-group">
              <label>Current members ({spaceMembers.length})</label>
              <div style={{ maxHeight: '150px', overflowY: 'auto', border: '1px solid hsl(var(--border))', borderRadius: 'var(--border-radius-md)', padding: '4px 10px' }}>
                {spaceMembers.map((member) => (
                  <div key={member.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: '1px solid hsl(var(--border) / 0.5)' }}>
                    ...
                    {member.id !== activeSpace.createdBy?.id && <button type="button" className="btn btn-secondary btn-sm" onClick={() => onRemove(member.id)}>Remove</button>}
                  </div>
                ))}
              </div>
            </div>
